...
class _VaultCompanionScreenState extends State<VaultCompanionScreen> {
  ...
  Future<void> _send() async {
    ...
    // keep cursor in the input (feels like iMessage)
    _focus.requestFocus();
    await _scrollToBottom();
    ...
    try {
      ...
      // replace typing placeholder with final assistant answer
      if (!mounted) return;
      setState(() {
        final idx = _msgs.lastIndexWhere((m) => m.role == _Role.assistant && m.isTyping);
        if (idx != -1) {
